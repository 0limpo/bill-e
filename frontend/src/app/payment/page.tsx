...
function PaymentPageContent() {
  ...
  // electrónica integration). Outside Chile we route to Polar.sh as MoR.
  ...
  // Geo gate — loader while detecting.
  ...
  // Chile: payments blocked until boleta electrónica integration is live.
  ...
  // International: route to Polar.sh hosted checkout.
  // Require sign-in first so the email Polar charges matches the email
  // used to grant premium via webhook.
  if (paymentRail === "international") {
    if (status === "need_auth" || !user?.email) {
      return (
        <div className="min-h-screen bg-background text-foreground flex flex-col">
          <div className="p-4 border-b border-border">
            <button
          ...
          <div className="flex-1 flex flex-col items-center justify-center px-6 text-center max-w-md mx-auto">
            ...
            <p className="text-muted-foreground text-sm mb-6">{t("payment.authRequiredDesc")}</p>
            <button
            ...
            <p className="text-xs text-muted-foreground mt-4">{t("payment.authBenefit")}</p>
          </div>
        </div>
      );
    }
